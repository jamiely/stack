...
- V3 distraction framework with deterministic seeded state, level-gated channel activation, runtime toggle/threshold tuning hooks, on-demand debug launch triggers, and V3.3 actor visuals (gorilla perimeter climbing with rhythmic slam pulses, UFO full-circuit tower orbit with off-screen fly-off exits toward the player + contrast wash, UFO orbit locked to one slab-height above the top floor with earlier default activation, persistent world-projected cloud cover with stack occlusion that keeps at least one cloud onscreen while enabled, tremor pulse, and intermittent bright-purple tentacle bursts that punch through all windows on one randomized slab side)
...
- V4.2 collapse fail sequence: hard misses trigger a deterministic tower-topple presentation with failure camera pullback, voxelized tower explosion burst (including the missed active slab), and collapse feedback cues
...
- Upward camera follow with frame-rate-independent smoothing + eased look-target tracking + tunable framing offset/direct Y offset, score/player-built-height HUD (excluding prebuilt starter slabs), impact flash + configurable placement shake feedback, vertically centered 3D window façades (body frames, protruding sills, deterministic per-side count variation, one consistent style per slab, rectangular/pointed-gothic/rounded-gothic/planter/shuttered variants), runtime guards that skip too-narrow faces and prevent shutter overlap while still allowing ledges/windows on viable narrow faces, randomized scalloped eave trims with face-length-driven sizing, bottom-half weathering on landed slabs (including prebuilt starters), and falling debris
