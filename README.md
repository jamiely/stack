...
- V3 distraction framework with deterministic seeded state, level-gated channel activation, runtime toggle/threshold tuning hooks, on-demand debug launch triggers, and V3.3 actor visuals (gorilla perimeter climbing with rhythmic slam pulses, UFO full-circuit tower orbit with off-screen fly-off exits toward the player + contrast wash, UFO orbit locked to one slab-height above the top floor, persistent world-projected cloud cover with stack occlusion that keeps at least one cloud onscreen while enabled, tremor pulse, and tentacle bursts that punch through all windows on one randomized slab side)
