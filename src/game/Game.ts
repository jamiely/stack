...
import type { CloudCameraFrame, CloudSimulationConfig, CloudState } from "./logic/clouds";
...
const DEBUG_RANGES: Record<DebugNumberKey, { min: number; max: number; step: number; label: string }> = {
  ...
  distractionFireworksLaunchIntervalMinSeconds: { min: 0.2, max: 6, step: 0.05, label: "Fireworks Launch Min" },
  distractionFireworksLaunchIntervalMaxSeconds: { min: 0.2, max: 6, step: 0.05, label: "Fireworks Launch Max" },
  distractionFireworksShellSpeedMin: { min: 1, max: 120, step: 1, label: "Fireworks Shell Speed Min" },
  distractionFireworksShellSpeedMax: { min: 1, max: 120, step: 1, label: "Fireworks Shell Speed Max" },
  distractionFireworksShellGravity: { min: 0, max: 200, step: 1, label: "Fireworks Gravity" },
  distractionFireworksShellTrailTicksMin: { min: 1, max: 240, step: 1, label: "Fireworks Trail Ticks Min" },
  distractionFireworksShellTrailTicksMax: { min: 1, max: 240, step: 1, label: "Fireworks Trail Ticks Max" },
  distractionFireworksSecondaryDelayMinSeconds: { min: 0, max: 4, step: 0.01, label: "Fireworks Secondary Delay Min" },
  distractionFireworksSecondaryDelayMaxSeconds: { min: 0, max: 4, step: 0.01, label: "Fireworks Secondary Delay Max" },
  distractionFireworksParticleLifetimeMinSeconds: { min: 0.1, max: 8, step: 0.05, label: "Fireworks Lifetime Min" },
  distractionFireworksParticleLifetimeMaxSeconds: { min: 0.1, max: 8, step: 0.05, label: "Fireworks Lifetime Max" },
  distractionFireworksMaxActiveParticles: { min: 32, max: 10000, step: 1, label: "Fireworks Max Particles" },
  distractionFireworksPrimaryParticleCount: { min: 1, max: 120, step: 1, label: "Fireworks Primary Count" },
  distractionFireworksSecondaryParticleCount: { min: 1, max: 120, step: 1, label: "Fireworks Secondary Count" },
  distractionFireworksRingBias: { min: 0, max: 1, step: 0.01, label: "Fireworks Ring Bias" },
  distractionFireworksRadialJitter: { min: 0, max: 1, step: 0.01, label: "Fireworks Radial Jitter" },
  distractionFireworksVerticalBias: { min: -1, max: 1, step: 0.01, label: "Fireworks Vertical Bias" },
  distractionFireworksSpeedJitter: { min: 0, max: 1, step: 0.01, label: "Fireworks Speed Jitter" },
...
const REMY_DUAL_SPAWN_SPREAD_RATIO = 0.18;
...
export class Game {
      ...
      ([key, meta]) => {
        ...
        input.addEventListener("input", () => {
          const integerKeys: DebugNumberKey[] = [
            ...
            "distractionFireworksShellSpeedMin",
            "distractionFireworksShellSpeedMax",
  ...
  private updateCloudLayer(snapshot: DistractionSnapshot, deltaSeconds: number): void {
    ...
    if (!this.cloudState || this.cloudState.clouds.length !== simulationConfig.count) {
      this.cloudState = initializeCloudState({
        seed: this.cloudSimulationSeed,
        config: simulationConfig,
        cameraFrame,
      });
    }

    this.cloudState = stepCloudState({
      previousState: this.cloudState,
      config: simulationConfig,
      cameraFrame,
      deltaSeconds,
    });
    ...
    this.cloudState.clouds.forEach((cloud, index) => {
      const cloudNode = cloudNodes[index];
      if (!cloudNode) {
        return;
      }

      const projected = new Vector3(cloud.x, cloud.y, topSlab.position.z + cloud.z).project(this.camera);
      if (!Number.isFinite(projected.x) || !Number.isFinite(projected.y) || !Number.isFinite(projected.z)) {
        cloudNode.style.opacity = "0";
        return;
      }

      const screenX = (projected.x * 0.5 + 0.5) * width;
      const screenY = (-projected.y * 0.5 + 0.5) * height;
      const variantScale = 0.82 + cloud.styleVariant * 0.09;
      const laneScale = cloud.lane === "front" ? 1.02 : 0.78;
      const scale = variantScale * laneScale;
  ...
  private syncCloudNodePool(count: number): HTMLElement[] {
    ...
    while (cloudNodes.length > targetCount) {
      const cloudNode = cloudNodes.pop();
      cloudNode?.remove();
    }
  ...
  private buildFireworksSimulationConfig(): FireworksConfig {
    return {
      ...
      primaryParticleCount: this.debugConfig.distractionFireworksPrimaryParticleCount,
      secondaryParticleCount: this.debugConfig.distractionFireworksSecondaryParticleCount,
      ringBias: this.debugConfig.distractionFireworksRingBias,
      radialJitter: this.debugConfig.distractionFireworksRadialJitter,
      verticalBias: this.debugConfig.distractionFireworksVerticalBias,
      speedJitter: this.debugConfig.distractionFireworksSpeedJitter,
      spawnXMin: -10,
      spawnXMax: 10,
      spawnZMin: -10,
      spawnZMax: 10,
  ...
  private createCloudCameraFrame(topSlab: SlabData): CloudCameraFrame {
    const reference = new Vector3(topSlab.position.x, topSlab.position.y, topSlab.position.z).project(this.camera);
    const depth = Math.max(-1, Math.min(1, reference.z));

    const topPoint = new Vector3(0, 1, depth).unproject(this.camera);
    const bottomPoint = new Vector3(0, -1, depth).unproject(this.camera);
    const leftPoint = new Vector3(-1, 0, depth).unproject(this.camera);
    const rightPoint = new Vector3(1, 0, depth).unproject(this.camera);

    return {
      viewTopY: Math.max(topPoint.y, bottomPoint.y),
      viewBottomY: Math.min(topPoint.y, bottomPoint.y),
      visibleWorldXMin: Math.min(leftPoint.x, rightPoint.x),
      visibleWorldXMax: Math.max(leftPoint.x, rightPoint.x),
    };
  }
