...
- V3.3 actor layer now renders a gorilla that follows a deterministic tower-climb path around the building with rhythmic slam pulses, a UFO that can complete a full tower orbit and then flies off-screen toward the player in Z instead of popping out when deactivating (with contrast-wash flashes), persistent front-layer cloud cover (including mid-screen occluders) with distraction-driven intensity boosts, intermittent tentacle bursts (now with deterministic purple/green/pink palettes) that persist on previously affected slabs instead of being replaced on each new placement and are further throttled with a minimum spawn interval, and simulation-driven fireworks with staged shell launch/primary/secondary/fade lifecycles plus isotropic burst-direction sampling and reduced particle drag/gravity shaping for a more spherical chrysanthemum silhouette; default fireworks morphology/count tuning now ships in a chrysanthemum-biased profile (`primary=48`, `secondary=8`, `ringBias=0.2`, `radialJitter=0.2`, `verticalBias=0.08`, `speedJitter=0.14`)
...
- Newly landed slabs now also spawn a dedicated lower ledge/balcony strip on a visible face; ledges animate in from narrow to full configured size and deterministically vary from 25% to 100% of the face span, and the Remy character is anchored onto a visible ledge with automatic world-space scaling so it stays appropriately sized as slab height changes (he does not hop to a newer ledge while his current ledge is still visible); Remy now loads a separate skinned character mesh (`assets/remy_character_t_pose.glb`) and animation clip file (`assets/remy_hip_hop_animation_inplace.glb`) with retarget fallback support, plays in ping-pong loop mode to smooth non-seam clip boundaries, auto-orients to the best detected up-axis for non-Y-up model exports, disables mesh frustum culling for reliability, falls back to top-slab placement if no ledge anchor is available, and is temporarily hidden only when tentacle bursts are present on that same ledge face to avoid clipping
...
- Remy placement: runtime rotation controls on all axes (X/Y/Z) plus translation controls on all axes (X/Y/Z) now map directly to the applied transform (with tuned defaults for this rig), and a `Reset Remy Placement` action for rapid model anchoring/orientation tuning
