...
- UFO orbit altitude tracks one slab-height above the current tower top center. The actor uses an optimized low-poly textured `Flying Saucer A` GLB in the Three.js scene, with bounded model width and a tighter world-space orbit so the craft stays readable beside the tower. Its textures, lightweight materials, and transparent fade shader are warmed before the model becomes ready to prevent a first-appearance frame hitch. The original CSS saucer remains as a fallback when WebGL, asset loading, or GPU warmup is unavailable, and default UFO start-level tuning remains low enough to appear in normal runs.
