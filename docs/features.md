...
- Collapse now also spawns a deterministic voxelized explosion burst from tower slabs for game-over presentation, always including the unplaced slab that caused a hard miss plus any still-visible trimmed debris pieces from earlier cuts; voxels are generated with more even cube-like sizing, distributed across slab heights so upper blocks remain represented, and capped to slabs within roughly two block-heights outside the current screen bounds
