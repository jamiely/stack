...
- Character ledge placement keeps each cast member horizontally centered while applying a modest per-character outward depth bias (stronger for Amy's wallward animation silhouette), leaving visible shelf behind the feet so dancers read as standing on the balcony instead of attached to the wall; the model lab retains an overhead camera plus green-ledge/red-character debug materials for verification.
