...
- Landed (non-active) slabs (including the prebuilt starting stack) add procedural 3D window facades with body-framed windows and protruding sills, deterministic per-side window-count variation, one deterministic style per slab (no mixed styles on a single block), and style variants for rectangular, pointed gothic, rounded gothic, planter-box, shuttered, and bay windows while keeping windows vertically centered; window generation skips too-narrow faces, enforces stronger edge clearance so windows stay away from block edges, clamps pair spacing to avoid excessive gaps while keeping the set horizontally centered, enforces shutter spacing so shutters never overlap, varies shutter colors per slab, and occasionally switches trim/sill palettes to dark/black accents
...
- Landed slabs can also render scalloped eave trims near top edges for arcade-style architectural variation, on all visible sides, with slight corner overlap to reduce visible corner gaps
