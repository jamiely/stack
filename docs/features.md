...
- Character ledge placement keeps each cast member horizontally centered while applying a modest per-character outward depth bias (stronger for Amy's wallward animation silhouette), leaving visible shelf behind the feet so dancers read as standing on the balcony instead of attached to the wall. Two-character lanes use a tighter center spread and clamp against the scaled character support footprint; if a ledge cannot safely support both footprints, placement falls back to one centered dancer. Narrow mobile viewports ease the camera target toward far-right dancers so the corrected side-ledge position stays fully on-screen. The deterministic mobile placement test now also enforces the apparent ledge-end inset, and the model lab retains an overhead camera plus green-ledge/red-character debug materials for verification.
