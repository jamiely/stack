...
- Dancer placement: runtime rotation controls on all axes (X/Y/Z) plus translation controls on all axes (X/Y/Z) now map directly to the applied transform (with tuned, side-aware defaults for this rig so left/right ledge placement reads more consistently), and a `Reset Remy Placement` action for rapid model anchoring/orientation tuning
