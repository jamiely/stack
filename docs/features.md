...
- UFO orbit altitude follows a raised path above the moving active slab, keeping the full bobbing saucer clear of the block instead of crossing its center plane. The actor uses an optimized low-poly textured `Flying Saucer A` GLB in the Three.js scene, with bounded model width, a tighter world-space orbit, and a capped world-Y lag after its start level so the craft drifts lower on screen as the camera climbs instead of staying pinned to one viewport band. Its textures, lightweight materials, and transparent fade shader are warmed before the model becomes ready to prevent a first-appearance frame hitch. The original CSS saucer remains as a fallback when WebGL, asset loading, or GPU warmup is unavailable, and default UFO start-level tuning remains low enough to appear in normal runs.
- The bat channel uses Quaternius's CC0 `Animated Monsters` bat as a self-contained, texture-free GLB. Its named `Bat_Flying` clip runs through a dedicated mixer while existing deterministic orbit state remains authoritative; runtime normalization now keeps the bat smaller than the UFO, the model yaws toward the camera for a readable silhouette, and its capped world-Y-lagged flight band is projected lower in the foreground below the UFO while keeping the real animated envelope separated from the active moving slab by foreground depth clearance. Transparent fades retain depth testing, and `data-model-state`/`data-model-visible` expose lifecycle state to E2E tests. The prior CSS bat remains visible during loading and after any WebGL/asset warmup failure.
...
- Placement diagnostics include a read-only overhead inspection camera and exact target-ledge material isolation; switching to that camera preserves the same character pair, transforms, and ledge anchors even on portrait mobile viewports, so side/overhead screenshots prove one unchanged scene.
...
- Portrait mobile viewports (aspect ratio `<= 0.75`) add six units of camera pullback, using effective distance 18 with the default tuning, while desktop and landscape viewports retain the default distance 12
...
- Active distraction channels drive visible actors (gorilla, Three.js UFO and animated bat models with CSS fallbacks, cloud layer, fireworks), contrast wash intensity, and camera tremor pulse effects; cloud rendering is world-projected so vertical cloud position tracks world space instead of appearing pinned to screen coordinates, and the cloud adapter maps deterministic lane/variant metadata to rounded-lobe silhouettes with subtle front/back depth styling.
- The runtime debug panel has an accessible `Hide debug` / `Show debug` control so debug mode remains usable on mobile without permanently covering the playfield.
...
- Newly landed slabs now also spawn a dedicated lower ledge/balcony strip on a visible face; ledges animate in from narrow to full configured size and deterministically vary from 25% to 100% of the face span, and dancer spawning is gated per ledge by a deterministic 50% chance. Active dancers remain anchored to their current ledge until that ledge leaves the viewport, then reroll onto the next eligible ledge. Character selection advances through `assets/remy_character_t_pose.glb`, `assets/timmy_tiny_webp.glb`, `assets/amy_tiny_webp.glb`, and `assets/aj_tiny_webp.glb` in strict next-up session rotation (full cast coverage before repeats, even across run resets), while animation selection randomizes across `assets/remy_hip_hop_animation_inplace.glb`, `assets/house_dancing_inplace.glb`, `assets/chicken_dance_inplace.glb`, and `assets/ymca_dance_inplace.glb` with immediate-repeat prevention on consecutive appearances. Ledges with width ratio >= `0.75` still place a two-character pair instead of one, and the pair uses the next two entries from that rotation (always different); clip playback retains retarget fallback support, strips animated bone-scale tracks and non-root bone-position tracks before playback to prevent limb-stretch artifacts across mixed-cast clips, ping-pong looping to smooth non-seam boundaries, auto-orientation to the best detected up-axis for non-Y-up model exports, disabled mesh frustum culling for reliability, fallback to top-slab placement only when no ledge anchor exists at all, balcony ledges are deepened enough for the full cast footprint to visibly read as standing on the shelf from front and side orientations, Amy and AJ keep zero model-space horizontal correction so their visible meshes remain centered on the logical lane anchor, placement moves dancers outward from the wall and lowers the contact point onto the balcony top surface, and temporary hide behavior only when tentacle bursts are present on that same ledge face to avoid clipping
- Decorative windows/eaves/weathering and tentacle bursts are all limited to camera-facing sides
- Character loading/normalization/placement helpers are now split out of `Game.ts` into focused `src/game/characters/*` modules so placement, scale, retargeting, and async load-coordination bugs can be debugged without reopening the main runtime monolith
- Character ledge placement keeps each cast member horizontally centered while applying only a shallow outward depth bias across the cast, so even minimum-width balconies retain balanced front/back support instead of visually stranding dancers near the outer edge. Two-character eligibility is resolved from the selected ledge's width metadata before character selection, and two-character lanes use a tighter center spread clamped against the scaled character support footprint; if that ledge cannot safely support both footprints, placement falls back to one centered dancer. Amy's animated pose is grounded every simulation tick against her actual visible foot position, using one precise mesh calibration followed by cached foot-bone anchors, so jump/bob keyframes cannot leave her floating above the balcony without imposing a per-frame full-mesh scan. Narrow mobile viewports ease the camera target toward far-right dancers so the corrected side-ledge position stays fully on-screen. The deterministic mobile placement test now also enforces the apparent ledge-end inset, and the model lab retains an overhead camera plus green-ledge/red-character debug materials for verification.
...
- Dancer placement: runtime rotation controls on all axes (X/Y/Z) plus translation controls on all axes (X/Y/Z) now map directly to the applied transform with per-character normalization defaults (`Remy: Rot Y=-9, Rot X=-7, Rot Z=89`; `Timmy: Rot Y=-180, Rot X=180, Rot Z=180`; `Amy: Rot Y=-180, Rot X=180, Rot Z=180`; `AJ: Rot Y=-180, Rot X=180, Rot Z=180`; all use `Move Y=0` so feet stay anchored to ledges), a `Reset Remy Placement` action for rapid model anchoring/orientation tuning, and model-lab toggles for `Show Spatial Helpers`, `Force Top Placement Lab`, plus `Overhead Ledge View` so manual QA can inspect placement/bounds/contact points without editing code
...
- Playwright end-to-end tests cover:
  ...
  - Gorilla/UFO/bat/cloud actor rendering activation, 3D model lifecycle visibility, and continued trim correctness while distractions are active
  - Debug on-demand distraction launch buttons forcing each channel (including bat and fireworks) active for a short runtime window
  ...
  - Character placement model-lab coverage for top-down red-character/green-ledge support, outward wall clearance, front-edge safety, and per-character ledge-footprint coverage, including every character on both front and right-side ledges from regular game spawning
