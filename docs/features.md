...
- The bat channel uses Quaternius's CC0 `Animated Monsters` bat as a self-contained, texture-free GLB. Its named `Bat_Flying` clip runs through a dedicated mixer while existing deterministic orbit state remains authoritative; runtime normalization bounds desktop and portrait scale, the model yaws toward the camera for a readable silhouette, its full bobbing envelope is routed above the active moving slab with explicit world-space clearance, transparent fades retain depth testing, and `data-model-state`/`data-model-visible` expose lifecycle state to E2E tests. The prior CSS bat remains visible during loading and after any WebGL/asset warmup failure.
