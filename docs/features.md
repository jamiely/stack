...
- Active distraction channels drive visible actors (gorilla, Three.js UFO model with CSS fallback, cloud layer, fireworks), contrast wash intensity, and camera tremor pulse effects; cloud rendering is world-projected so vertical cloud position tracks world space instead of appearing pinned to screen coordinates, and the cloud adapter maps deterministic lane/variant metadata to rounded-lobe silhouettes with subtle front/back depth styling.
