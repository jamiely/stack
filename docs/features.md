...
- UFO orbit altitude follows a raised path above the moving active slab, keeping the full bobbing saucer clear of the block instead of crossing its center plane. The actor uses an optimized low-poly textured `Flying Saucer A` GLB in the Three.js scene, with bounded model width and a tighter world-space orbit so the craft stays readable beside the tower. Its textures, lightweight materials, and transparent fade shader are warmed before the model becomes ready to prevent a first-appearance frame hitch. The original CSS saucer remains as a fallback when WebGL, asset loading, or GPU warmup is unavailable, and default UFO start-level tuning remains low enough to appear in normal runs.
- The bat channel uses Quaternius's CC0 `Animated Monsters` bat as a self-contained, texture-free GLB. Its named `Bat_Flying` clip runs through a dedicated mixer while existing deterministic orbit state remains authoritative; runtime normalization bounds desktop and portrait scale, the model yaws toward the camera for a readable silhouette, and its flight band is projected in the lower/foreground screen area below the UFO while keeping the real animated envelope separated from the active moving slab by foreground depth clearance. Transparent fades retain depth testing, and `data-model-state`/`data-model-visible` expose lifecycle state to E2E tests. The prior CSS bat remains visible during loading and after any WebGL/asset warmup failure.
