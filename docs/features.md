...
- Character ledge placement now keeps each cast member's visible footprint centered on the ledge depth/width for overhead inspection, with a model-lab overhead camera plus green-ledge/red-character debug material mode for self-verifying whether dancers actually sit on balcony shelves.
...
- Playwright end-to-end tests cover:
  ...
  - Character placement model-lab coverage for top-down overhead red-character/green-ledge centering and per-character ledge-footprint support
