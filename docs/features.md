...
- UFO orbit altitude tracks one slab-height above the current tower top center. The actor now uses the low-poly textured `Flying Saucer A` GLB in the Three.js scene, with bounded model width and a tighter world-space orbit so the craft stays readable beside the tower. The original CSS saucer remains as a fallback when WebGL or asset loading is unavailable, and default UFO start-level tuning remains low enough to appear in normal runs.
