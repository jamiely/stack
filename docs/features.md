...
- Newly landed slabs now also spawn a dedicated lower ledge/balcony strip on a visible face; ledges animate in from narrow to full configured size and deterministically vary from 25% to 100% of the face span, and the Remy dancing GLB character is anchored onto a visible ledge with automatic world-space scaling so it stays appropriately sized as slab height changes (he does not hop to a newer ledge while his current ledge is still visible); Remy auto-orients to the best detected up-axis for non-Y-up model exports, disables mesh frustum culling for reliability, falls back to top-slab placement if no ledge anchor is available, and is temporarily hidden only when tentacle bursts are present on that same ledge face to avoid clipping
